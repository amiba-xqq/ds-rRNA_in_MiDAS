...
dir=/mnt/LJW/CUT_TAG_Pol_202505
...
cd ${dir}/bam_sort/
ls *rep1.rmdupsorted.bam  >1
ls *rep2.rmdupsorted.bam  >2
paste 1 2 >config
cat config | while read id
do
    arr=($id)
    bam1=${arr[0]}
    bam2=${arr[1]}
    samtools merge -o ../bam_merge/$(basename -s .rmdupsorted.bam $bam1).merged.bam $bam1 $bam2
done
